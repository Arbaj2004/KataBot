...
const AdminActiveTickets = () => {
    const token=localStorage.getItem('token')
    const navigate = useNavigate();
    const [allBlogs, setAllBlogs] = useState([]);
    const fetchAllBlogs = async () => {
        const URL = `${import.meta.env.VITE_APP_BACKEND_URL}/admin/getAllActiveTickets`;
        try {
    ...
    <div>AdminSolveTicket
        ...
        {allBlogs.map((blog) => (
            ...
            <div className="p-4 flex flex-col justify-between h-full">
              ...
              <p className="text-gray-500 text-sm">{formatDate(blog.createdAt)} &bull;{blog.EmailID} </p>
            </div>
          </div>
        ))}
      </div>
...
export default AdminActiveTickets